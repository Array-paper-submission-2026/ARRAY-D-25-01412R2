"""
Main pipeline for resume summarization experiments.

This script:
1. Loads configuration
2. Finds supported resume files recursively
3. Generates summaries with selected models
4. Saves outputs to model-specific directories
5. Records per-model runtime statistics
...
class Pipeline:
    """End-to-end pipeline for summary generation and efficiency reporting."""
    ...
    def _load_config(self, config_path: str) -> dict:
        """
        Load YAML configuration file. If unavailable, use defaults.
        """
        try:
    ...
    def _print_setup_summary(self) -> None:
        ...
        print(f"[INFO] Results directory: {self.results_dir.resolve()}")
    ...
    def _build_output_path(self, file_path: Path, model_name: str) -> Path:
        """
        Build output path for a summary file.

        Args:
            file_path (Path): Original resume file path
            model_name (str): Summarization model name

        Returns:
            Path: Output path for generated summary
        """
    ...
    def _build_efficiency_report(self, timing_results: dict, processed_counts: dict) -> pd.DataFrame:
        """
        Build efficiency summary table.

        Returns:
            pandas.DataFrame: Per-model timing statistics
        """
        efficiency_rows = []

        for model_name, durations in timing_results.items():
            count = processed_counts.get(model_name, 0)
            model_type = (
                "abstractive"
                if model_name in self.models.abstractive_models
                else "extractive"
    ...
    def run(self) -> None:
        ...
        processed_counts = {model: 0 for model in models_to_process}
        ...
        print(f"[INFO] Summary generation complete. Outputs saved to: {self.output_dir.resolve()}")

        efficiency_df = self._build_efficiency_report(timing_results, processed_counts)
        ...
        efficiency_file = self.results_dir / "efficiency_summary.csv"
        try:
            efficiency_df.to_csv(efficiency_file, index=False, float_format="%.3f")
            print(f"[INFO] Efficiency report saved to: {efficiency_file.resolve()}")
        except Exception as e:
            print(f"[WARNING] Could not save efficiency report: {e}")

        print("\n[INFO] Notes:")
        print("- Timing reflects summarization function calls only.")
        print("- Timing excludes text extraction, preprocessing, file I/O, and model loading.")
        print("- Runtime characteristics may differ across CPU and GPU environments.")
